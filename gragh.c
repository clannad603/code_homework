...
void CreatMGraph(MGraph *G) {
    ...
    for (k = 0; k < G->e; k++) {
        int z;//读入e条边，建立邻接矩阵
        scanf("%d%d%d", &i, &j,&z);        //输入边（Vi，Vj）的顶点序号
        G->edges[i][j] = z;
        G->edges[j][i] = z; //若为无向图，矩阵为对称矩阵；若建立有向图，去掉该条语句
    }
}
...
void prime(MGraph *G){
    ...
    for (int i = 1; i < G->n ; ++i) {
        ...
        printf("%c-%c :%d\n", adjvex[min_id]+'0', min_id+'0', min );
        lowCost[min_id] = 0;
        for (int j = 1; j < G->n; ++j) {
            if (G->edges[min_id][j] < lowCost[j] &&lowCost[j]!=0){
                lowCost[j] = G->edges[min_id][j];
                adjvex[j] = min_id;
            }
        }
